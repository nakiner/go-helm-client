...
func newClient(options *Options, clientGetter genericclioptions.RESTClientGetter, settings *cli.EnvSettings) (Client, error) {
	...
	if options.SlogHandler != nil {
		slogHandler = options.SlogHandler
	} else {
		slogHandler = slog.NewTextHandler(options.Output, &slog.HandlerOptions{
...
func mergeInstallOptions(chartSpec *ChartSpec, installOptions *action.Install) {
	...
	installOptions.HideNotes = chartSpec.HideNotes
	installOptions.HideSecret = chartSpec.HideSecret
	installOptions.SkipSchemaValidation = chartSpec.SkipSchemaValidation
	installOptions.EnableDNS = chartSpec.EnableDNS
...
func mergeUpgradeOptions(chartSpec *ChartSpec, upgradeOptions *action.Upgrade) {
	...
	upgradeOptions.HideNotes = chartSpec.HideNotes
	upgradeOptions.HideSecret = chartSpec.HideSecret
	upgradeOptions.SkipSchemaValidation = chartSpec.SkipSchemaValidation
	upgradeOptions.EnableDNS = chartSpec.EnableDNS
}
